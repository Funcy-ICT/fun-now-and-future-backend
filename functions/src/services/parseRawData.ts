...
export const parseRawData = (a: string): ParsedAdvertisement => {
    let companyId: string | null = null;// Manufacturer Specific Dataが見つからなかった場合はnullのまま返す。
    let isNearbyInfo: boolean = false;
        ...
        while (now_location < a.length / 2) {
            ...
            if (!Number.isInteger(length) || length === 0) break;// [長さ]が0の場合、[型][中身]が存在しないので、その時点でパースを終了する。データが途中で切れているとparseIntがNaNを返すため、あわせて終了する。
            ...
            if (datatype === "FF") {// [型]が0xffの場合、[中身]はCompany Specific Dataとわかる。
                const id = (getByte(a, now_location + 3) + getByte(a, now_location + 2)).toUpperCase();// Company IDはリトルエンディアンで格納されているため、2バイトを入れ替えて結合する。
                companyId = id.length === 4 ? id : null;// データが途中で切れていると2桁以下になるため、4桁揃っている場合のみ採用する。
                if (companyId === "004C") {// CompanyIDが"0x004C"の場合、Specific dataがApple continutityとわかる。
                    ...
                    while (apple_location < end) {
                        ...
                        if (!Number.isInteger(apple_length)) break;// 中身が途中で切れている場合、これ以上は読めないので終了する。
                        if (apple_type === "10") {// Apple continutityの[型]が0x10の場合、[中身]はNearby Infoとわかる。0x07(AirPods等)や0x12(AirTag)はアクセサリーのため対象外。
                            isNearbyInfo = true;
                        }
